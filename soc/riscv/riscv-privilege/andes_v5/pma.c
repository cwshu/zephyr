...
typedef struct pma_region_attr pma_region_attr_t;

struct pma_region {
	ulong_t start;
	ulong_t size;
	pma_region_attr_t attr;
};

/* pmaaddr supports 4-byte granularity in each mode */
#define ADDR_DIV_4(addr)            ((addr) >> 2)

/* The base address is aligned to size */
#define NAPOT_BASE(start, size)     ADDR_DIV_4((start) & ~((size) - 1))
/* The encoding of size is 0b01...1 (change the leading bit of bitmask to 0) */
#define NAPOT_SIZE(size)            ADDR_DIV_4(((size) - 1) >> 1)

#define NA4_ENCODING(start)         ADDR_DIV_4(start)
#define NAPOT_ENCODING(start, size) (NAPOT_BASE(start, size) | NAPOT_SIZE(size))
...
{
	switch(index) {
		case 0:  write_csr(NDS_PMAADDR0,  value); break;
		case 1:  write_csr(NDS_PMAADDR1,  value); break;
		case 2:  write_csr(NDS_PMAADDR2,  value); break;
		case 3:  write_csr(NDS_PMAADDR3,  value); break;
		case 4:  write_csr(NDS_PMAADDR4,  value); break;
		case 5:  write_csr(NDS_PMAADDR5,  value); break;
		case 6:  write_csr(NDS_PMAADDR6,  value); break;
		case 7:  write_csr(NDS_PMAADDR7,  value); break;
		case 8:  write_csr(NDS_PMAADDR8,  value); break;
		case 9:  write_csr(NDS_PMAADDR9,  value); break;
		case 10: write_csr(NDS_PMAADDR10, value); break;
		case 11: write_csr(NDS_PMAADDR11, value); break;
		case 12: write_csr(NDS_PMAADDR12, value); break;
		case 13: write_csr(NDS_PMAADDR13, value); break;
		case 14: write_csr(NDS_PMAADDR14, value); break;
		case 15: write_csr(NDS_PMAADDR15, value); break;
	}
}

/*
 * Write value to pma{i}cfg registers which are packed into CSRs pmacfg{j}
 */
static void write_pmacfg_entry(const uint32_t entry_index, uint8_t entry_value)
{
	...
	uint32_t index = entry_index >> RV_REGSHIFT;
	uint8_t  shift = (entry_index % RV_REGSIZE) * 8;
	/* In riscv64, CSR pmacfg index are even number (0, 2) */
	index <<= (RV_REGSHIFT - 2);

	ulong_t pmacfg = 0;
	switch(index) {
		case 0: read_csr(pmacfg, NDS_PMACFG0); break;
		case 1: read_csr(pmacfg, NDS_PMACFG1); break;
		case 2: read_csr(pmacfg, NDS_PMACFG2); break;
		case 3: read_csr(pmacfg, NDS_PMACFG3); break;
	}
	...
	switch(index) {
		case 0: write_csr(NDS_PMACFG0, pmacfg); break;
		case 1: write_csr(NDS_PMACFG1, pmacfg); break;
		case 2: write_csr(NDS_PMACFG2, pmacfg); break;
		case 3: write_csr(NDS_PMACFG3, pmacfg); break;
	}
}
...
static void configure_nocache_region()
{
 ...
 *
 * In SMP, each CPU has it's own PMA CSR and PMA only affect one CPU.
 * We should configure all CPUs' PMA CSR to make memory region really
 * (e.g. uncacheable) to all CPUs.
 */
void pma_init_per_core()
{
#ifdef CONFIG_NOCACHE_MEMORY
